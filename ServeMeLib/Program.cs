        ...
        private static void Main(string[] args)
        {
            ...
            if (!new WindowsPrincipal(WindowsIdentity.GetCurrent()).IsInRole(WindowsBuiltInRole.Administrator))
                ...
                using (server = new ServeMe())
                {
                    ...
                    var ret = server.CanOpenDefaultBrowserOnStart(urls[0]);
                    if (ret!=null)
                    { Process.Start(ret);}
        ...
        private static bool RunInstruction(string entry)
        {
            ...
            if (entry?.ToLower() == "init")
            {
                ...
                File.WriteAllLines(file, new List<string>()
                {
                    ...
                    "***# app log",
                    "",
                    "*** Log everything that happens to text file like 'log.txt' do",
                    "***# app log, log.txt",
                    "",
                    ...
                    "",
                    "*** To execute a function 'MyFun(x)' inside a dll do",
                    "***# StartsWith /runSomeFun,assembly MyDll.dll MyClassName  MyFun myArgX,get",
                    "",
                    "*** To save the result of a request if status is a 200, into file 'data.json' do",
                    "getSome,http://www.google.com,get,200,save data.json",
                    "",
                    "*** To save the result of a request using the same filename as the response i.e {{file}}, do",
                    "***# startswith /api , https://www.xxxxxx.com{{pathandquery}},get,200,save path/to/{{file}}.json",
                    "",
                    "*** To return content or data.json when GET or POST /GetOrders do ",
                    "***# GetOrders , data.json",
                    "",
                    "*** To combile the obove two features using 'memo' ( memoization ) where 'path' is the location where you want to save the files, do ",
                    "***# startswith /api , https://www.google.com{{pathandquery}},get,200,memo path",
                    "***# OR SIMPLY DO",
                    "***# startswith /api , https://www.google.com{{pathandquery}},memo path",
                    "***# OR PASS QUERY STRING VARIABLES AND USE THEM AS ID LIKE THIS",
                    "***# startswith /api?images=true&like=false , https://www.google.com{{pathandquery}},memo path | {{&images}}{{&like}}",
                    "",
                    "******************** OTHER HELPFUL HINTS ********",
                    "***",
                    ...
                    "***",
                    "***************************************************",
                    "******************** RECIPE TO OPEN DEFAULT BROWSER ON STARTUP, YOU CAN SUPPLY THE PATH RELATIVE TO THE HOST ********",
                    "***",
                    "*** app dir, path/to/directory",
                    "*** app openDefaultBrowserOnStartUp, path/to/anotherDirectory/index.html",
                    "*** app log",
                    "***",
                    "***************************************************",
                ...
                .Select(
                    webpage =>
                    {
                        try
                        {
                            ...
                            {
                                object res = SimpleHttpServer.Execute(null, entry,null,null);
                            ...
                            else if (executionType == "sourcecode")
                            {
                                ...
                                object res = SimpleHttpServer.Execute(null,source,null,null);
                                Console.BackgroundColor = ConsoleColor.White;
                                Console.ForegroundColor = ConsoleColor.Black;
                                TrySaveResult(saveLocation, res == null ? "" : new JavaScriptSerializer().Serialize(res));
                                // if (printResult)
